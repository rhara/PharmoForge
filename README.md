...
`pf`コマンド(CLI)化は現在保留中。`src`以下の関数群を充実させ、[notebooks](notebooks/README.md)
配下でテーマを持ったJupyter notebookとして活用例をまとめる方針にシフトしている。
...
| `src/seqalign` | 構造を伴わない任意アミノ酸配列同士のペアワイズグローバルアラインメント(Biopython `PairwiseAligner`)。当初`sequencealign`固有だったがアトミックな技術要素として独立させた |
...
| [`pf analyze-scaffolds`](scaffoldanalyzer/README.md) | Bemis-Murckoスキャフォールド単位での活性高低分布の比較 | `pf analyze-scaffolds data/cdk4_human_activities.tsv --outdir data/cdk4_scaffold_analysis` |
...
| [`pf find-pocket`](pocketfinder/README.md) | fpocketによるポケット検出・ポケット周辺残基の一覧化(JSON出力) | `pf find-pocket data/cdk2/P24941_AF.cif --outdir data/cdk2/P24941_AF_pockets` |
| [`pf view-pocket`](pocketfinder/README.md) | `pf find-pocket`のポケット一覧をPyMOLで強調表示 | `pf view-pocket data/cdk2/P24941_AF.cif --pockets data/cdk2/P24941_AF_pockets/pockets.json` |
| [`pf show-pocket`](pocketfinder/README.md) | `pf find-pocket`のポケット一覧を表形式(TSV)で出力 | `pf show-pocket data/cdk2/P24941_AF_pockets/pockets.json` |
