...
class AlignmentResult:
    ...
    query_resnum_by_ref_pos: dict[int, int]  # 基準配列内の位置(1始まり)→対応するquery側の残基番号(ギャップの位置は含まない)
...
def align_to_reference(ref_sequence: str, query_sequence: str, query_resnums: list[int]) -> AlignmentResult:
    ...
    query_resnum_by_ref_pos: dict[int, int] = {}
    aligned_length = 0
    n_identical = 0
    prev_ref_end = 0
    prev_query_end = 0
    for (ref_start, ref_end), (q_start, q_end) in zip(ref_blocks, query_blocks):
        ...
        for offset in range(ref_end - ref_start):
            ...
            query_resnum_by_ref_pos[ref_i + 1] = query_resnums[q_i]
    ...
    return AlignmentResult(
        ...
        query_resnum_by_ref_pos=query_resnum_by_ref_pos,
    )
